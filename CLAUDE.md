...
**Careful with `--fhir` outside cpp2sqlite: it is inverted.** `zurrose`, `pharma` and `sai` do `bool flagFHIR = false; if (!vm.count("fhir")) flagFHIR = true;` (`src/zur/main.cpp:199`, and the same in `pha`/`sai`), so *omitting* the flag selects FHIR and *passing* `--fhir` selects the legacy BAG XML — the opposite of `cpp2sqlite`, where `--no-fhir` is the opt-out. To exercise `bag.cpp` you must pass `--fhir` to these three and `--no-fhir` to cpp2sqlite.
