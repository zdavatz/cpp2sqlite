...
**Verifying a change here:** build both versions and compare the databases, not just row counts. Normalise the build timestamp in the generated footer (`Auto-generated by ... on <date>`), then hash every column of every row in `_id` order. Two runs of unchanged code hash identically, so any real difference shows up. Cover the options that change output — `--lang de`, `--lang fr`, `--pinfo`, `--no-fhir` (19-column legacy schema), `--without-sappinfo` — because `bag.cpp` only runs under `--no-fhir`.
